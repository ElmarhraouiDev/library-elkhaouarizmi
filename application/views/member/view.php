                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="profile_view_item">
                                        <p><b><?=$this->lang->line('member_classe')?></b>: <?=calculate($classe) ? $classe->classe : ''?></p>
                                    </div>
                                    <div class="profile_view_item">
                                        <p><b><?=$this->lang->line('member_class_group')?></b>: <?=$member->class_group?></p>
                                    </div>
                                    <!-- <div class="profile_view_item">
                                    ...
                                    <div class="profile_view_item">
                                        <p><b><?=$this->lang->line('member_place_birth')?></b>: <?=$member->placeofbirth?></p>
                                    </div>
                                    <div class="profile_view_item">
                                                ...
                                                <tr>
                                                    ...
                                                    <?php 
                                                                ...
                                                                if($bookissue->status == 0 && $date_current > $date_bookissue_exp) {
                                                                    $status = $this->lang->line('member_issued_delayed');              
                                                                    $status_color = "red";              
                                                                } elseif($bookissue->status == 0) {
                                                                    $status = $this->lang->line('member_issued');              
                                                                } elseif ($bookissue->status == 1 && ($bookissue->paidstatus != 2) && ($bookissue->fineamount > 0)) {
                                                                    $status = $this->lang->line('member_return'); 
                                                                    $status_color = "red";             
                                                                } elseif ($bookissue->status == 1) {
                                                                    $status = $this->lang->line('member_return');              
                                                                } elseif ($bookissue->status == 2) {
                                                                    $status = $this->lang->line('member_lost');
                                                                }
                                                            ?>    
                                                        <span class="text-bold text-success" style="color: <?= $status_color ?>"> <?= $status ?> </span>
                                                    </td>

                                                    <?php if(permissionChecker('bookissue_view') || permissionChecker('bookissue_edit') || permissionChecker('member_delete')) { ?>
                                                        <td data-title="<?=$this->lang->line('member_action')?>">
                                                            ...
                                                            <?php if(($bookissue->status == 0) && ($bookissue->deleted_at == 0) && ($bookissue->renewed == 1)) { 
                                                                echo btn_edit('bookissue/edit/'.$bookissue->bookissueID, $this->lang->line('member_edit')). " ";
                                                                echo btn_delete('bookissue/delete/'.$bookissue->bookissueID, $this->lang->line('member_delete'));
                                                            } ?>
                                                            ...
                                                            <?php if($bookissue->status == 0) { ?>
                                                                ...
                                                                <a href="<?=base_url('bookissue/returnbook/'.$bookissue->bookissueID)?>" class="btn btn-info btn-xs mrg" style="background-color: #8900cf;border-color: #9227c9;" onclick="return confirm('Do you really want to return the book. This cannot be undone. are you sure?')" data-placement="auto" data-toggle="tooltip" data-original-title="<?=$this->lang->line('member_return')?>"><i class="fa fa-undo"></i></a>
                                                            <?php } ?>
                            
                                                            <?php if(permissionChecker('bookissue_add') && ($bookissue->paidstatus != 2) && ($bookissue->fineamount > 0)) { ?>
                                                                <span data-toggle="tooltip" data-original-title="<?=$this->lang->line('member_payment')?>"><button class="btn btn-mytheme btn-xs mrg paymentamount" data-bookissueid="<?=$bookissue->bookissueID?>" data-placement="auto" data-toggle="modal" data-target="#paymentmodal"><i class="fa fa-money"></i></button></span>
                                                            <?php } ?>
