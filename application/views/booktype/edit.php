						...
						<div class="box-body">
							<div class="form-group <?=form_error('name') ? 'has-error' : ''?>">
							 	<label for="name"><?=$this->lang->line('booktype_name')?></label> <span class="text-red">*</span>
							  	<input type="text" class="form-control" id="name" name="name" value="<?=set_value('name', $booktype->name)?>" placeholder="Enter name">
							  	<?=form_error('name')?>
							</div>
							<div class="form-group <?=form_error('description') ? 'has-error' : ''?>">
							  	<label for="description"><?=$this->lang->line('booktype_description')?></label> <span class="text-red">*</span>
							  	<textarea name="description" id="description" cols="30" rows="5" class="form-control" placeholder="Enter description"><?=set_value('description', $booktype->description)?></textarea>
							...
							<div class="form-group <?=form_error('bookissue_type') ? 'has-error' : ''?>">
							  	...
							  	<?php 
									...
									echo form_dropdown('bookissue_type', $bookissue_typeArray, set_value('bookissue_type', $booktype->bookissue_type),'id="bookissue_type" required class="form-control"');
								?>
							...
							<div id="form_fixdate" class="form-group <?=form_error('bookissue_date') ? 'has-error' : ''?>" <?=form_error('bookissue_date') ? '' : 'style="display: none;'?> >
							 	<label for="bookissue_date"><?=$this->lang->line('bookissue_date')?></label><span class="text-red">*</span>
							  	<input type="text" class="form-control datepicker" id="bookissue_date" name="bookissue_date" value="<?=set_value('bookissue_date', $booktype->bookissue_date)?>" placeholder="Enter book issue date">
							  	<?=form_error('bookissue_date')?>
							</div>
						</div>
						<div class="box-footer">
							<button type="submit" class="btn btn-mytheme"><?=$this->lang->line('booktype_update_booktype')?></button>
						</div>
					</form>
				</div>
			</div>
